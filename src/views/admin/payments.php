...
foreach ($payments as $p) {
    ...
    $aksi = '<div class="flex gap-1 items-center">'
        ...
        . '<button type="button" ' . ($isApproved ? Dialog::make()->id($invoiceId)->getOpenAttr() : '') . ' ' . ($isApproved ? '' : 'disabled') . ' class="text-gray-700 bg-gray-100 hover:bg-gray-200 ' . $btnBase . ($hasInvoice ? ' ring-2 ring-green-400' : '') . ' ' . ($isApproved ? '' : $dis) . '" data-tooltip="' . ($isApproved ? 'Atur Invoice' : 'Menunggu verifikasi') . '">' . Icon::make()->name('more-horizontal')->class('w-5 h-5') . '</button>'
        . '</div>';

      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            ...
            <h2 class="text-xl font-bold text-gray-800"><?= htmlspecialchars($team['name']) ?></h2>
            <p class="text-sm text-gray-500"><?= htmlspecialchars($team['teamSchool']) ?> &bull; Divisi: <span class="font-semibold text-brand"><?= htmlspecialchars($divisionUpper) ?></span></p>
          </div>
          <?php if ($info && !empty($info['guide_book'])): ?>
            <a href="<?= htmlspecialchars($info['guide_book']) ?>" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold text-white bg-brand hover:bg-red-800 rounded-xl transition-colors no-underline shrink-0">
        ...
        <?php if ($info): ?>
          ...
          <!-- Video Trial Section (if available, e.g., FFR) -->
          ...
          <!-- Timeline Divisi -->
          <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            <h3 class="text-md font-bold text-gray-800 mb-4">Timeline Activity Divisi <?= htmlspecialchars($divisionUpper) ?></h3>
            <div class="relative border-l-2 border-brand/20 ml-4 space-y-6">
              <?php foreach ($info['timeline'] as $item): ?>
                <div class="relative pl-6">
                  <div class="absolute -left-[9px] top-1 w-4 h-4 rounded-full bg-brand border-4 border-white"></div>
                  <h4 class="text-sm font-bold text-gray-800"><?= htmlspecialchars($item['title']) ?></h4>
                  <p class="text-xs text-gray-500 mt-0.5"><?= htmlspecialchars($item['date']) ?></p>
                </div>
              <?php endforeach; ?>
            </div>
          </div>
